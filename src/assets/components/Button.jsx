...
export function Button(props){
    let basicProps = processProps(props, generalStyles.button,
        ...
        content = (props.icon) ? (<div class={generalStyles.iconButtonText}>{props.children}</div>) : props.children;
    if(props.type == "link"){
        return (<Link href={props.href} class={basicProps.class} style={basicProps.style}>{icon}{content}</Link>);
    }else if(props.type == "action"){
        if(typeof props.function != "function")
            throw new Error("Invalid <Button> action!");
        return (<button onClick={props.function} class={basicProps.class} style={basicProps.style}>{icon}{content}</button>);
        ...
        return (<button class={basicProps.class} style={basicProps.style}>{icon}{content}</button>);
    }
}
